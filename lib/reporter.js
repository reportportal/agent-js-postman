...
class Reporter {
    ...
    onConsole (err, args) {
        ...
        switch (type) {
            ...
            case 'suite':
                ...
                break;
            default:
                args.messages.forEach(message => this.logs.push({ level: args.level, message, time: this.getTime() }));
        }
    }
    ...
    onBeforeRequest (err, result) {
        ...
        this.logs && this.logs.forEach(log => this.logMessage(testObj.tempId, log.message, log.level, log.time));
        this.logs = [];
