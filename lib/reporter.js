...
const { PREDEFINED_LOG_LEVELS } = require('./constants/logLevels');

const LEVEL_LOG = 'log';

/**
 * @typedef {PREDEFINED_LOG_LEVELS | string} LOG_LEVELS
 * Log level type: can be a predefined level (TRACE, DEBUG, WARN, INFO, ERROR, FATAL) or any custom string.
 */
...
class Reporter {
  ...
  onConsole(err, args) {
    ...
    const level =
      args.level === LEVEL_LOG ? args.messages[2] || PREDEFINED_LOG_LEVELS.INFO : args.level;
    const messages = args.messages ? args.messages.slice(1) : [];
  ...
  finishStep(error, testAssertion) {
    ...
    if (actualError) {
      ...
      this.logMessage(stepObj.tempId, actualError.message, PREDEFINED_LOG_LEVELS.ERROR);
    }

    let { status } = currentStep;

    if (!status) {
      if (testAssertion.skipped) status = TestStatus.SKIPPED;
      else if (actualError) status = TestStatus.FAILED;
  ...
  finishTest(err, result) {
    ...
    if (testObj.error) {
      this.logMessage(testObj.testId, testObj.error.message, PREDEFINED_LOG_LEVELS.ERROR);
    ...
    const hasFailedAssertions =
      result.assertions && result.assertions.some((assertion) => assertion.error);
   ...
   * @param  {LOG_LEVELS} [level=PREDEFINED_LOG_LEVELS.INFO] Log level. Can be a predefined level (TRACE, DEBUG, WARN, INFO, ERROR, FATAL) or any custom string.
   * @param  {number} time when the log was started.
   */
  logMessage(id, value, level = PREDEFINED_LOG_LEVELS.INFO, time) {
    this.client
      .sendLog(id, {
        level,
        message: value,
  ...
  interruptAllSteps(testObj, message) {
    _.forEach(testObj.steps, (step) => {
      this.logMessage(step.stepId, message, PREDEFINED_LOG_LEVELS.ERROR);
