...
class Reporter {
    ...
    onStart (err, result) {
        ...
        const suiteObj = this.client.startTestItem({
            ...
            description,
            codeRef,
            testCaseId: utils.getValueByKey('testCaseId', this.collectionRunOptions.collection.variables),
    ...
    onBeforeRequest (err, result) {
        ...
        const testObj = this.client.startTestItem({
            ...
            codeRef,
            parameters,
            testCaseId: utils.getValueByKey('testCaseId', this.collectionRunOptions.environment.values),
        ...
        this.collectionMap.set(result.cursor.ref, {
            ...
            steps: [],
            status: utils.getValueByKey('status', this.collectionRunOptions.environment.values)
        });
    ...
    onBeforeTest (err, result) {
        ...
        _.filter(result.events, 'script')
            ...
            .flatMap(exec => {
                ...
                return { ...stepName && { stepName }, ...testCaseId && { testCaseId }, ...status && { status } };
            })
    ...
    onDone (err, result) {
        ...
        const status = this.collectionRunOptions.collection &&
            utils.getValueByKey('launchStatus', this.collectionRunOptions.collection.variables);
    ...
    finishSuite () {
        ...
        const status = this.collectionRunOptions.collection &&
            utils.getValueByKey('status', this.collectionRunOptions.collection.variables);
