...
module.exports = function (emitter, options, collectionRunOptions) {
    ...
    function finishSteps (reference, testObj, run) {
        ...
        const requestStep = _.find(testObj.steps, 'requestId'),
            execution = _.find(run.executions, { id: requestStep.requestId });
        ...
        const scriptFailure = _.find(run.failures, { at: 'test-script', cursor: { ref: reference } });
        ...
        _.forEach(testObj.steps, step => {
            const failure = _.find(run.failures, { cursor: { ref: reference }, error: { test: step.name } });
    ...
    function getTags (variables) {
        const tags = _.find(variables.members, { key: 'tags' });
