...
module.exports = function (emitter, options, collectionRunOptions) {
    ...
    emitter.on('beforeTest', (err, o) => {
        ...
        _.filter(o.events, event => event.script)
            ...
            .forEach(script => {
                ...
                testObj.steps.set(scriptId, []);

                _.forEach(script.exec, exec => {
                    ...
                    testObj.steps.get(scriptId).push({
                        stepName: stepName,
    ...
    function finishSteps(ref, testObj, failures) {
        testObj.steps.forEach((steps, scriptId) => {
            _.map(steps, step => {
                let failed =
                    _.filter(failures, failure => failure.cursor.ref == ref && failure.cursor.scriptId == scriptId)
                    .some(failure => !failure.error || failure.error.test == step.stepName);
    ...
    function finishTest(ref, testObj, failures) {
        let failed = _.some(failures, failure => failure.cursor.ref == ref);
