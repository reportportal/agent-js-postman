...
module.exports = function (emitter, options, collectionRunOptions) {
    ...
    emitter.on('beforeDone', (err, o) => {
        ...
        // Test run object
        let run = o.summary.run;

        collectionMap.forEach((value, key) => {
            finishSteps(key, value, run);
            finishTest(key, value, run);
        });
    });
    ...
    function sendRequestLogs(stepId, request) {
        ...
        let headers = request.headers.members.map(header => `${header.key}:${header.value}`);
        if (headers.length) {
    ...
    function sendResponseLogs(stepId, response) {
        ...
        let headers = response.headers.members.map(header => `${header.key}:${header.value}`);
    ...
    function finishSteps(ref, testObj, run) {
        ...
        let requestStep = testObj.steps.find(step => step.requestId);
        let response = run.executions.find(exec => exec.id == requestStep.requestId).response;

        sendResponseLogs(requestStep.stepId, response);

        // Check for failures
        let scriptFailure = _.find(run.failures, failure => failure.at == "test-script");
        ...
        if (scriptFailure) {
            ...
            testObj.steps.forEach(step => {
                let failure = _.find(run.failures, failure => failure.cursor.ref == ref && failure.error.test == step.name);
    ...
    function finishTest(ref, testObj, run) {
        let failed = _.some(run.failures, failure => failure.cursor.ref == ref);
    ...
    function getStepName(execScript) {
        ...
        let stepName = execScript.match(/^[\s]*pm.test\(\"(.*)\"/) || execScript.match(/^[\s]*tests\[\"(.*)\"\]/);
        return stepName == null ? null : stepName[1];
    }
}