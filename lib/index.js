...
module.exports = function(emitter, options, collectionRunOptions) {
    ...
    emitter.on("start", err => {
        ...
        // Array of collection variables
        const collectionVars =
            collectionRunOptions.collection.variables.members;

        launchObj = client.startLaunch({
            description: description,
            tags: getTags(collectionVars)
        });
    ...
    emitter.on("beforeRequest", (err, o) => {
        ...
        const envVars = collectionRunOptions.environment.values.members;

        const testObj = collectionMap.get(o.cursor.ref);
        const stepName = o.request.description
            ? o.request.description.content
            : "Postman request";
    ...
    function sendRequestLogs(stepId, request) {
        ...
        const headers = request.headers.members.map(
    ...
    function sendResponseLogs(stepId, response) {
        ...
        const headers = response.headers.members.map(
            header => `${header.key}:${header.value}`
        );
    ...
    function finishSteps(ref, testObj, run) {
        ...
        const requestStep = _.find(testObj.steps, step => step.requestId);
        const execution = _.find(
            run.executions,
            exec => exec.id == requestStep.requestId
        );
        ...
        const scriptFailure = _.find(
            run.failures,
            failure => failure.at == "test-script" && failure.cursor.ref == ref
        );
        ...
        _.forEach(testObj.steps, step => {
            const failure = _.find(
                run.failures,
                failure =>
                    failure.cursor.ref == ref && failure.error.test == step.name
            );
    ...
    function finishTest(ref, testObj, run) {
        const failed = _.some(
            run.failures,
            failure => failure.cursor.ref == ref
        );
     ...
     * of tags.
     *
     * @param  {Array} variables Array of postman's variables.
     * @returns {Array} Array of tags.
     */
    function getTags(variables) {
        const tags = _.find(variables, variable => variable.key == "tags");
