...
module.exports = function (emitter, options, collectionRunOptions) {
    ...
    emitter.on('start', err => {
        ...
        launchObj = client.startLaunch({
            ...
            rerun: options.rerun,
            rerunOf: options.rerunOf,
            attributes: getAttributes(collectionRunOptions.collection.variables)
        });
    ...
    emitter.on('beforeRequest', (err, o) => {
        ...
        const testObj = collectionMap.get(o.cursor.ref),
            ...
            stepObj = client.startTestItem({
                name: stepName,
                type: 'STEP',
                attributes: getAttributes(collectionRunOptions.environment.values)
            },
            launchObj.tempId,
            testObj.testId);
     ...
     */
    function getAttributes (variables) {
        const attributes = _.map(variables.members, member => ({
            key: member.key,
            value: member.value
        }));
