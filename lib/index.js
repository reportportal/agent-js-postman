...
>>>>>>> fc89e45508f05af80e3c10d0b80f8e8178db1456
/**
    ...
    emitter.on('beforeTest', (err, o) => {
        ...
        _.filter(o.events, event => event.script)
            ...
            .forEach(exec => {
                ...
                // Starts a new step for every test in test script
                getStepNames(exec).forEach(stepName => {
                    const stepObj = client.startTestItem({
                        name: stepName,
                        type: 'STEP'
                    }, launchObj.tempId, testObj.testId);
...
<<<<<<< HEAD
        ...
        return script.split(';')
            ...
            .map(x => utils.matchPattern(x, testPatterns, 1))
=======
        return script.split(';')
            .sliceOn(0, x => x.includes('//')) // Removes commented elements
